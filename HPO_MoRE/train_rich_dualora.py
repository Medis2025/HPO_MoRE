...
"""
Span-level HPO-ID Contrastive Training (with enriched HPO JSON + field-tagged gold text)

- 利用真实 NER 标注 (GeneReviews / GSC+ / ID-68) + enriched hpo_enriched_with_llm.json
- 构造 mention span ↔ gold HPO span 对
- gold HPO 文本使用带 field 标记的结构：
    [HPO_ID] HP:0002199
    [NAME] Hypocalcemic seizures
    [SYN] Low calcium seizures; seizures due to hypocalcemia.
    [DEF] ...
    [ADD_DEF] ...
...
python /cluster/home/gw/Backend_project/NER/process_bert/add_head/HPO/NER_H/pipeline_remote/HPO_MoRE/train_rich_dualora.py \
  --train_roots \
  ...
  --out_dir /cluster/home/gw/Backend_project/NER/tuned/duallora_span_llm \
  --epochs 50 --batch_size 16 --max_len 512
"""
...
# =============================================================================
# HPO gold sentence builder (with field tags)
# =============================================================================
def _extract_primary_name(rec: Dict) -> str:
    """
    从 enriched HPO 记录中抽取主 Name：
      - 优先 rec["Name"][0] (list)
      - 退而求其次 rec["name"] 或 "label"
      - 不存在时，返回空字符串（上游会 fallback 到 hpo_id）
    """
    name = rec.get("Name") or rec.get("name") or rec.get("label") or ""
...
def _extract_synonyms(rec: Dict, max_syn: int = 3) -> List[str]:
    syns = rec.get("Synonym") or rec.get("synonym") or []
    if isinstance(syns, str):
        syns = [syns]
    syns = [s.strip() for s in syns if isinstance(s, str) and s.strip()]
    if max_syn > 0:
        syns = syns[:max_syn]
    return syns


def _extract_original_def(rec: Dict) -> str:
    d = rec.get("Def") or rec.get("def") or rec.get("definition") or rec.get("Definition") or ""
...
def _extract_llm_defs(rec: Dict) -> Tuple[str, str]:
    llm_def = (rec.get("llm_def") or "").strip()
    llm_add = (rec.get("llm_add_def") or "").strip()
    return llm_def, llm_add
...
def build_hpo_gold_sentence_and_span(
    ...
    """
    使用 enriched HPO JSON + 带 field 标记构造 gold_text + name_span：
      ...
      gold_text 形如：
        ...
        [NAME] Hypocalcemic seizures
        [SYN] Low calcium seizures; seizures due to hypocalcemia.
        [DEF] Seizures occurring in the setting of low serum calcium concentrations.
        [ADD_DEF] LLM: ...

      name_span 为 gold_text 中 Name 部分的字符范围 (start, end)，
      即上面示例中 "Hypocalcemic seizures" 在整个字符串里的位置。

    逻辑：
      - Name：优先 ontology.data[hid]["Name"][0]，否则用 hpo_id 兜底
      - Syn：来自 Synonym（最多 max_syn 个）
      - Def：优先原始 Def；缺失时 fallback 到 llm_def
      - Add_def：使用 llm_add_def（若存在）
    """
    hid = ontology.resolve_id(hpo_id)
    rec = ontology.data.get(hid, {}) or {}

    # --- 主 Name ---
    name = _extract_primary_name(rec)
    if not name:
        name = hid  # 兜底

    # --- 同义词 ---
    syns = _extract_synonyms(rec, max_syn=max_syn)
    syn_str = "; ".join(syns) if syns else ""

    # --- 定义 & LLM 补充 ---
    orig_def = _extract_original_def(rec)
    llm_def, llm_add = _extract_llm_defs(rec)

    # 主定义：优先原始 Def，其次 llm_def
    if orig_def:
        def_line = f"[DEF] {orig_def}"
    elif llm_def:
        def_line = f"[LLM_DEF] {llm_def}"
    else:
        def_line = ""

    add_line = f"[ADD_DEF] {llm_add}" if llm_add else ""

    # --- 按顺序组装 field-tagged 文本 ---
    parts: List[str] = []
    parts.append(f"[HPO_ID] {hid}")
    parts.append(f"[NAME] {name}")

    if syn_str:
        parts.append(f"[SYN] {syn_str}")
    if def_line:
    ...
    # --- 计算 name_span：在整个 gold 文本中第一次出现 name 的位置 ---
    # 例如：
    #   [HPO_ID] HP:0002199\n[NAME] Hypocalcemic seizures\n...
    #             ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    #             这里的 "Hypocalcemic seizures" 的起止位置
    idx = gold.find(name)
    if idx == -1:
        # 极端情况（几乎不会发生），fallback 到整段
        name_span = (0, len(gold))
    else:
        name_span = (idx, idx + len(name))
...
class HPOIDSpanPairDataset(Dataset):
    ...
    - 使用 enriched HPOOntology: 构造 field-tagged gold sentence + name span

    每个样本：
        {
          "left_text": str,        # mention 上下文
          "left_span": (c0,c1),    # left_text 内的 span
          "gold_text": str,        # HPO gold sentence (with field tags)
    ...
    def __init__(
        ...
        for root in roots:
            ...
            if not (os.path.isdir(ann_dir) and os.path.isdir(corpus_dir)):
                logger.warning(
                    f"[HPOIDSpanPairDataset] Skip root (missing ann/corpus): {root}"
                )
                continue
            ...
            for ann_path in sorted(glob.glob(os.path.join(ann_dir, "*"))):
                ...
                if not os.path.isfile(text_path):
                    ...
                    if os.path.isfile(alt):
                        ...
                        logger.warning(
                            f"[HPOIDSpanPairDataset] Missing corpus for ann: {ann_path}"
                        )
                        continue
                ...
                for span, mention, hid_raw in anns:
                    ...
                    # 右侧：gold sentence + name span（使用 field tags 和 enriched JSON）
...
def encode_spans(
    ...
    # forward through NER encoder (mode 由外部控制：train / eval)
    out = model_tc(
        input_ids=input_ids,
...
def encode_hpo_gold_table(
    ...
    """
    为一组 HPO ID 构建 field-tagged gold sentence + name span，并 encode 得到 [N_hpo, D] 向量表。

    ⚠️ 这里不缓存到全局，只返回给调用方（如 evaluate_hpoid）临时使用，用完即丢。
    """
    texts: List[str] = []
    spans: List[SPAN] = []
    valid_ids: List[str] = []
    ...
    if not texts:
        return torch.empty(
            0, span_proj.proj.out_features, device=device
        ), []
...
# =============================================================================
def info_nce_loss(
    z_left: torch.Tensor, z_right: torch.Tensor, temperature: float = 0.05
) -> torch.Tensor:
    """
    z_left  : [B, D]
    z_right : [B, D]
...
def evaluate_hpoid(
    ...
    - 对「验证集里真正出现过的 HPO 子集」构建临时 gold table（右边）
    - 计算每个 span 对该子集 HPO gold 的 cos 相似度
    - 统计 top-1 / top-5 正确率

    ⚠️ 关键点：
      - 只用 val 中出现的 HPO 子集（避免全库）
      - z_hpo / id2idx 只在本函数内部存在，用完即丢，不挂在全局，避免长驻显存占用
    """
    model_tc.eval()
    span_proj.eval()
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Span-level HPO-ID contrastive training using NER encoder + enriched hpo.json"
    )
    ap.add_argument(
        "--train_roots",
        type=str,
    ...
    ap.add_argument(
        ...
        type=str,
        required=True,
        help="Path to enriched HPO JSON (e.g., hpo_enriched_with_llm.json)",
    )
    ap.add_argument(
        "--model_dir",
        type=str,
...
def main():
    ...
    # Ontology (用 enriched HPO JSON)
    ontology = HPOOntology(args.hpo_json)
    logger.info(
        f"Loaded HPO ontology from {args.hpo_json} with {len(ontology.data)} nodes."
    )
    ...
    span_proj = SpanProj(
        in_dim=hidden_size, out_dim=args.hpoid_dim, dropout=0.1
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Eval（每个 epoch 使用“子集临时建表 + 用完即丢”策略）
