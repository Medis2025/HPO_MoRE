...
python /cluster/home/gw/Backend_project/NER/process_bert/add_head/HPO/NER_H/pipeline_remote/HPO_MoRE/HPO_fuzzy_llm.py \
  --eval_roots \
  ...
  --out_dir /cluster/home/gw/Backend_project/NER/process_bert/add_head/HPO/NER_H/VAL_LLM_fuzzy \
...
# -------------------------------------------------------------------------
# Helper: extract HPO info for prompt  (REVISED to use llm_def / llm_add_def)
# -------------------------------------------------------------------------
def get_hpo_prompt_info(ontology: HPOOntology, hpo_id: str) -> Dict[str, Any]:
    """
    Map HPOOntology data into a structure for LLM, using enriched fields:

      - 原始字段: Name / Def / Synonym
      - 新字段:  llm_def / llm_add_def  (来自 hpo_enriched_with_llm.json)
    ...
    其中 hpo_def 采用统一格式：
        ...
        [HPO_ID] HP:0001250
        [NAME] Seizures
        [SYN] Epileptic seizures; Convulsions
        [DEF] 原始定义...
        [LLM_DEF] LLM 生成的补充定义...
        [ADD_DEF] LLM 基于 PubMed/MedGen 的扩展描述...

    这样 LLM 的 prompt 侧就可以利用更丰富、结构化的定义信息。
    """
    ...
    return {
        "hpo_id": hid,
        "hpo_name": name,
        "hpo_def": combined_def,      # LLM prompt 主体用这个
        "hpo_synonyms": syns,         # 保留原字段，兼容旧 prompt 逻辑
        # 额外暴露原始字段，若后续 prompt 想单独用：
...
def build_candidates_with_duallora(   # ← 名字保留不变，避免 main() 需要修改
    dataset_name: str,
    ds: HPOIDSpanPairDataset,
    model_tc,
    span_proj,
    ...
    device,
    z_hpo,               # <== 不再使用，但保留函数签名
    hpo_ids_vec,         # <== 全局 HPO ID 列表
    id2idx,              # <== 不再使用
    topk: int = 15,
):
    """
    *** Fuzzy-only Candidate Retrieval ***
    Replace DualLoRAEnc encoder-based retrieval with:
    
      - fuzzy string matching between mention text and HPO Name
      - similarity = fuzz.partial_ratio(mention, name)
      - keep top-K results as candidates
      - margin = score_top1 - score_top2
    
    All other logic remains IDENTICAL to the original pipeline.
    """

    logger.info(
        f"[Fuzzy] Building top-{topk} fuzzy candidates for dataset={dataset_name} "
        f"over {len(hpo_ids_vec)} HPO terms..."
    )

    total = 0
    dual_top1_hits = 0          # now "fuzzy top1"
    dual_recallK_hits = 0       # now "fuzzy recall@K"
    samples_for_llm = []

    # Preload all HPO names for fuzzy matching
    hpo_name_map = {}
    for hid in hpo_ids_vec:
        rec = ontology.data.get(hid, {})
        raw_name = rec.get("Name") or rec.get("name") or rec.get("label")
        if isinstance(raw_name, list):
            name = raw_name[0]
        else:
            name = raw_name if isinstance(raw_name, str) else hid
        hpo_name_map[hid] = name

    # Iterate dataset
    for ex in tqdm(ds, desc=f"[Fuzzy] {dataset_name} spans", leave=False):
        total += 1
        ...
        # Compute fuzzy score for each HPO
        scores = []
        for hid in hpo_ids_vec:
            name = hpo_name_map.get(hid, "")
            score = fuzz.partial_ratio(mention.lower(), name.lower())
            scores.append((hid, float(score)))

        # Sort by score desc
        scores.sort(key=lambda x: x[1], reverse=True)
        top_items = scores[:topk]

        # fuzzy top1
        fuzzy_top1 = top_items[0][0] if top_items else None
        ...
        # fuzzy recall@K
        fuzzy_ids = {hid for hid, _ in top_items}
        gold_in_topk = gold in fuzzy_ids
        if gold_in_topk:
            dual_recallK_hits += 1

        # Only generate LLM samples when gold ∈ topK
        if not gold_in_topk:
            continue

        # Construct candidate list with definitions
        candidates = []
        ...
        if len(top_items) >= 2:
            margin = top_items[0][1] - top_items[1][1]
        else:
            margin = top_items[0][1]
        ...
        sample = {
            ...
            "dual_best_id": fuzzy_top1,
            "dual_margin": float(margin),  # still called dual_margin to avoid pipeline changes
        }
    ...
    logger.info(
        f"[Fuzzy] dataset={dataset_name}: top1={dual_top1:.4f}, "
...
def parse_args():
    ...
    ap.add_argument(
        ...
        type=str,
        required=True,
        help="Path to hpo.json (here: enriched_with_llm.json).",
    )
    ap.add_argument(
        "--model_dir",
        type=str,
...
def main():
    ...
    # Ontology (从 enriched_with_llm.json 载入，内部 data 已包含 llm_def / llm_add_def)
