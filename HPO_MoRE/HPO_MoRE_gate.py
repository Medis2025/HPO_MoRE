...
DualLoRAEnc + Fuzzy + LLM refine eval for HPO-ID with margin-based gating:
...
Step 2: For spans where the gold HPO is in top-K, apply margin-based gating
        with an additional fuzzy expert on the top-K candidates:

    Let dual_margin = dual_score_top1 - dual_score_top2.

    - If dual_margin >= tau_high   (easy case):
        -> Trust DualLoRAEnc top-1 directly, DO NOT call fuzzy or LLM.

    - Else:
        * Compute fuzzy scores on the SAME top-K candidate list
          using mention text vs HPO name only (small candidate set):

              fuzzy_score = SequenceMatcher(mention, hpo_name).ratio()

          Let fuzzy_margin = fuzzy_score_top1 - fuzzy_score_top2.

        * If fuzzy_margin >= tau_fuzzy_high (fuzzy is confident):
              -> Trust fuzzy top-1, DO NOT call LLM.

        * Else (Dual not highly confident, fuzzy also not confident):
              -> Fall back to LLM gating on the same top-K candidates:

                If dual_margin <= tau_low    (hard case):
                    -> Call LLM and trust its top choice among the K candidates.

                If tau_low < dual_margin < tau_high (medium case):
                    -> Call LLM:
                        * If LLM hits gold, accept LLM.
                        * If LLM misses, fall back to DualLoRAEnc top-1.

Thus the final prediction is a hybrid of DualLoRAEnc + fuzzy + LLM,
with LLM only used on spans where both Dual and fuzzy are not confident.

Additionally:
  - Compute **global** metrics across all datasets:
      * DualLoRAEnc global top-1
      * DualLoRAEnc global recall@K
      * Fuzzy global conditional top-1 (only on samples where fuzzy was used)
      * LLM global conditional top-1 (only on samples where LLM was actually called)
      * Full pipeline global top-1 (Dual+Fuzzy+LLM with margin gating)
  - Save:
      * JSON summary
...
python HPO_MoRE_gate.py \
  ...
  --out_dir /cluster/home/gw/Backend_project/NER/process_bert/add_head/HPO/NER_H/VAL_LLM_gate \
...
# -------------------------------------------------------------------------
# Helper: fuzzy score based on HPO name vs mention only
# -------------------------------------------------------------------------
def fuzzy_score(a: str, b: str) -> float:
    """
    Simple normalized fuzzy score between two strings using difflib.
...
# -------------------------------------------------------------------------
# DualLoRAEnc: build top-K candidates + basic metrics (with margin + fuzzy)
# -------------------------------------------------------------------------
def build_candidates_with_duallora(
    ...
    - Build LLM/fuzzy input samples ONLY for spans where gold ∈ top-K:
            ...
            "candidates": [      # sorted by Dual score desc
                {
                  ...
                  "hpo_def": ...,
                  "hpo_synonyms": [...],
                  "score": float,           # DualLoRAEnc similarity score
                }, * topK
            ],
            "dual_best_id": hpo_id of top1,
            "dual_margin": score_top1 - score_top2 (or 0 if no top2),

            # Fuzzy expert is computed ONLY on these top-K candidates:
            "fuzzy_scores": [float] * topK,             # mention vs hpo_name
            "fuzzy_margin": fuzzy_top1 - fuzzy_top2,
          }, ...
        ]
    """
    ...
    for i in tqdm(
        ...
        for row, (ex, gold) in enumerate(zip(chunk, gold_ids)):
            ...
            # Only construct samples for spans where gold ∈ topK
            if gold_in_topk:
                cand_list = []
                ...
                if cand_list:
                    ...
                    dual_margin = best_score - second_score
                else:
                    dual_best_id = None
                    dual_margin = 0.0

                # Fuzzy scores ONLY on this small candidate set
                fuzzy_scores: List[float] = []
                for cand in cand_list:
                    hpo_name = cand.get("hpo_name", "")
                    s = fuzzy_score(mention_text.lower(), hpo_name.lower())
                    fuzzy_scores.append(float(s))
...
# -------------------------------------------------------------------------
# LLM refine with Dual + Fuzzy margin-based gating + multithreading
# -------------------------------------------------------------------------
def run_llm_refine_for_dataset(
    ...
    tau_low: float = 0.05,
    tau_high: float = 0.20,
    tau_fuzzy_high: float = 0.20,
) -> Dict[str, float]:
    """
    Run Dual+Fuzzy+LLM refinement with margin-based gating on a dataset:

      For each sample (gold ∈ topK):

        dual_margin = sample["dual_margin"]
        dual_best_id = sample["dual_best_id"]
        fuzzy_scores = sample["fuzzy_scores"]
        fuzzy_margin = sample["fuzzy_margin"]

        1) If dual_margin >= tau_high (easy case):
               -> Prediction = dual_best_id
               -> NO fuzzy gating, NO LLM CALL.

        2) Else (Dual not highly confident):
             - If fuzzy_margin >= tau_fuzzy_high (fuzzy confident on top-K):
                   -> Prediction = fuzzy top-1
                   -> NO LLM CALL.
                   -> counted as a fuzzy call for fuzzy conditional stats.

             - Else:
                   -> Hard/medium case: use original LLM gating on top-K:

                      * Call LLM:
                           indices = refiner.refine(context, mention, candidates)
                         If indices valid:
                           llm_hid = candidates[indices[0]]["hpo_id"]
                      * If dual_margin <= tau_low: trust LLM (hard case).
                      * If tau_low < dual_margin < tau_high (medium case):
                             if llm_hid == gold: use LLM
                             else: fallback to dual_best_id

    We track:
      - pipeline_top1_hits: final hits of the combined pipeline
      - fuzzy_calls/fuzzy_hits: when fuzzy is used as the **final** decision
      - llm_calls/llm_hits: when LLM is actually invoked and used for stats
    ...
    Returns:
      {
        ...
        "fuzzy_calls": int,
        "fuzzy_top1_hits": int,
        "fuzzy_top1_conditional": float,  # hits / fuzzy_calls (0 if fuzzy_calls==0)
        "llm_calls": int,
        "llm_top1_hits": int,
    ...
    if not samples_for_llm:
        logger.warning(f"[LLM] dataset={dataset_name} has no samples for refine.")
        return {
            "pipeline_top1": 0.0,
    ...
    logger.info(
        f"[LLM] Starting Dual+Fuzzy+LLM margin-gated refine on dataset={dataset_name} "
        f"with {len(samples_for_llm)} samples..."
    )
    ...
    def _run_one(sample: Dict[str, Any]):
        ...
        Returns:
          (pipeline_hit: bool,
           llm_called: bool, llm_hit: bool,
           fuzzy_called: bool, fuzzy_hit: bool)
        """
        ...
        dual_margin = float(sample.get("dual_margin", 0.0))
        fuzzy_scores = sample.get("fuzzy_scores", [])
        fuzzy_margin = float(sample.get("fuzzy_margin", 0.0))
        ...
        # -----------------------------
        # 1) Dual very confident: trust Dual, no fuzzy / LLM
        # -----------------------------
        if dual_margin >= tau_high:
            pipeline_hit = (dual_best_id == gold_id)
            return pipeline_hit, False, False, False, False

        # -----------------------------
        # 2) Dual not highly confident: try fuzzy on top-K
        # -----------------------------
        if fuzzy_scores:
            # Fuzzy best index within top-K candidates
            best_fz_idx = max(range(len(fuzzy_scores)), key=lambda i: fuzzy_scores[i])
            fuzzy_best_id = candidates[best_fz_idx]["hpo_id"]
        else:
            fuzzy_best_id = dual_best_id
            fuzzy_margin = 0.0

        if fuzzy_margin >= tau_fuzzy_high:
            # Fuzzy confident on top-K -> trust fuzzy, no LLM
            pred_final = fuzzy_best_id
            pipeline_hit = (pred_final == gold_id)
            fuzzy_called = True
            fuzzy_hit = pipeline_hit
            return pipeline_hit, False, False, fuzzy_called, fuzzy_hit

        # -----------------------------
        # 3) Dual not confident, fuzzy not confident -> call LLM (original gating)
        # -----------------------------
        llm_called = True
        llm_hit = False
        fuzzy_called = False
        ...
        try:
            ...
            pipeline_hit = (pred_final == gold_id)
            return pipeline_hit, llm_called, llm_hit, fuzzy_called, fuzzy_hit
    ...
    with ThreadPoolExecutor(max_workers=max(1, num_workers)) as pool:
        ...
        for fut in tqdm(
            ...
            try:
                pipeline_hit, llm_called, llm_hit, fuzzy_called, fuzzy_hit = fut.result()
            except Exception as e:
    ...
    if llm_calls > 0:
        ...
        llm_cond = 0.0
    if fuzzy_calls > 0:
        fuzzy_cond = fuzzy_hits / fuzzy_calls
    else:
        fuzzy_cond = 0.0
    ...
    logger.info(
        ...
        f"fuzzy_calls={fuzzy_calls}, fuzzy_cond_top1={fuzzy_cond:.4f} (fuzzy_hits={fuzzy_hits}), "
    ...
    return {
        ...
        "n_samples": int(n),
        "fuzzy_calls": int(fuzzy_calls),
        "fuzzy_top1_hits": int(fuzzy_hits),
        "fuzzy_top1_conditional": float(fuzzy_cond),
...
def plot_comparisons(
    ...
    plt.bar(
        ...
        pipe_top1_vals,
        width=width,
        label="Pipeline (Dual+Fuzzy+LLM gating) Top-1",
    )
    plt.xticks(x, dataset_names, rotation=30, ha="right")
    plt.ylim(0.0, 1.0)
    plt.ylabel("Top-1 Accuracy")
    plt.title("Top-1 Comparison: DualLoRAEnc vs Dual+Fuzzy+LLM (margin-gated)")
    plt.legend()
    plt.tight_layout()
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="DualLoRAEnc eval + Fuzzy + LLM refine for HPO-ID (topK candidates) with margin-based gating (using global HPO table)."
    )
    ap.add_argument(
        "--eval_roots",
        type=str,
    ...
    ap.add_argument("--topk", type=int, default=15, help="topK candidates for refine.")
    # LLM related
    ap.add_argument(
        "--prompt_path",
        type=str,
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.20,
        help="Fuzzy margin threshold on top-K (fuzzy_margin >= tau_fuzzy_high -> trust fuzzy, no LLM).",
    )
...
def main():
    ...
    for root in eval_roots:
        ...
        # 3) Dual+Fuzzy+LLM refine with margin-based gating (only on gold ∈ topK samples)
        llm_metrics = run_llm_refine_for_dataset(
            ...
            tau_fuzzy_high=args.tau_fuzzy_high,
        )
        ...
        results_summary[dataset_name] = {
            ...
            "fuzzy_calls": llm_metrics["fuzzy_calls"],
            "fuzzy_top1_hits": llm_metrics["fuzzy_top1_hits"],
            "fuzzy_top1_conditional": llm_metrics["fuzzy_top1_conditional"],
        ...
        fuzzy_calls_all += llm_metrics["fuzzy_calls"]
        fuzzy_top1_hits_all += llm_metrics["fuzzy_top1_hits"]
    ...
    if fuzzy_calls_all > 0:
        global_fuzzy_conditional = fuzzy_top1_hits_all / fuzzy_calls_all
    else:
        global_fuzzy_conditional = 0.0
    ...
    results_summary["_GLOBAL"] = {
        ...
        "fuzzy_top1_conditional": float(global_fuzzy_conditional),
        ...
        "fuzzy_calls": int(fuzzy_calls_all),
        "fuzzy_top1_hits": int(fuzzy_top1_hits_all),
    ...
    logger.info(
        ...
        "Fuzzy conditional top1=%.4f (on fuzzy-called samples), "
        "LLM conditional top1=%.4f (on LLM-called samples), "
        "Pipeline top1=%.4f (total_spans=%d)",
        global_dual_top1,
        args.topk,
        global_dual_recallK,
        global_fuzzy_conditional,
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# HPO DualLoRAEnc + Fuzzy + LLM Refine Evaluation\n")
        f.write("*(Margin-Gated Pipeline on Global HPO Table)*\n\n")
        ...
        f.write(
            "- Fuzzy Conditional Top-1 (on spans where fuzzy gating was used): "
            "**{:.4f}**\n".format(g.get("fuzzy_top1_conditional", 0.0))
        )
        f.write(
            "- LLM Conditional Top-1 (on spans where LLM was called, given gold ∈ top-{}): "
            "**{:.4f}**\n".format(args.topk, g.get("llm_top1_conditional", 0.0))
        )
        f.write(
            "- **Full Pipeline Top-1 "
            "(DualLoRAEnc + Fuzzy gating + LLM refine, global HPO table)**: "
        ...
        f.write(
            "| Dataset | Dual Top-1 | Dual Recall@{} | Total spans | LLM samples | Fuzzy calls | Fuzzy Top-1 (cond.) | LLM calls | LLM Top-1 (cond.) | Pipeline Top-1 |\n".format(
                args.topk
            )
        )
        f.write(
        ...
        f.write(
            ...
            "and the margin-gated pipeline that combines DualLoRAEnc top-K retrieval with a fuzzy expert "
            "(mention vs HPO name on the candidate set) and LLM-based candidate selection. "
            "Dual similarity margin is first used to skip easy cases, then fuzzy-margin gating further "
            "filters confident lexical matches within the top-K, and only the remaining ambiguous spans "
