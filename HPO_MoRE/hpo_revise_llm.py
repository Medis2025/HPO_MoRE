...
Step 1: DualLoRAEnc (span encoder) does dataset-specific ALL-HPO retrieval
        on each dataset (GeneReviews / GSC+ / ID-68), and:
...
python hpo_revise_llm.py \
  ...
  --out_dir /cluster/home/gw/Backend_project/NER/process_bert/add_head/HPO/NER_H/VAL_LLM \
...
# -------------------------------------------------------------------------
# DualLoRAEnc: build dataset-specific HPO table
# -------------------------------------------------------------------------
...
def build_candidates_with_duallora(
    ...
    - Use DualLoRAEnc (z_left @ z_hpo^T) to do ALL-HPO retrieval on this dataset's HPO subset.
    - Compute:
    ...
    for i in tqdm(
        ...
        # Full-table retrieval: z_left @ z_hpo^T
        ...
        for row, (ex, gold) in enumerate(zip(chunk, gold_ids)):
            ...
            # Index of gold in dataset HPO table
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="DualLoRAEnc eval + LLM refine for HPO-ID (topK candidates) with margin-based gating."
    )
    ap.add_argument(
        "--eval_roots",
        type=str,
...
def main():
    ...
    for root in args.eval_roots:
        ...
        # 1) Build dataset-specific HPO table
        z_hpo, hpo_ids_vec, id2idx = build_dataset_hpo_table(
            ds,
            model_tc,
            span_proj,
            tokenizer,
        ...
        # 2) DualLoRAEnc: build top-K candidates & basic metrics
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# HPO DualLoRAEnc + LLM Refine Evaluation (Margin-Gated Pipeline)\n\n")

        f.write("## Command\n\n```bash\n")
        f.write("python hpo_revise_llm.py \\\n")
        ...
        f.write("- DualLoRAEnc Top-1: **{:.4f}**\n".format(g.get("dual_top1", 0.0)))
        f.write(
            "- DualLoRAEnc Recall@{}: **{:.4f}**\n".format(
        ...
        f.write(
            "- **Full Pipeline Top-1 (DualLoRAEnc + LLM refine with gating)**: "
        ...
        f.write(
            "\nThis report summarizes the performance of pure DualLoRAEnc retrieval and the "
            "margin-gated pipeline that combines DualLoRAEnc top-K retrieval with LLM-based "
