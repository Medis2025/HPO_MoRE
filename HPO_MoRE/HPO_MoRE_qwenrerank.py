...
What changed vs your HPO_MoRE.py:
✅ Replaced remote LLM API (LLMAPIClient + HPOCandidateRefiner) with a local CE reranker:
   /cluster/home/gw/Backend_project/models/Qwen3-Reranker-4B
✅ Refine now = pointwise cross-encoder scoring:
   score_i = P("yes" | instruct, query, candidate_i)
   pred = argmax_i score_i
✅ Gating logic preserved (Dual margin):
   - margin >= tau_high  -> trust Dual (no refine)
   - margin <= tau_low   -> call reranker, trust reranker
   - tau_low < margin < tau_high -> call reranker, accept reranker only if reranker is confident
     (confidence = rerank_margin >= --rerank_tau, else fallback to Dual)

Metrics:
- DualLoRAEnc top1 / recall@K over ALL spans
- Reranker conditional top1 over called samples (within gold_in_topk subset)
- Full pipeline top1 over ALL spans (Dual+Rerank gating)

Example:

python /cluster/home/gw/Backend_project/NER/process_bert/add_head/HPO/NER_H/pipeline_remote/HPO_MoRE/HPO_MoRE_qwenrerank.py \
  --eval_roots \
  ...
  --out_dir /cluster/home/gw/Backend_project/NER/process_bert/add_head/HPO/NER_H/VAL_QWEN_RERANK \
  --topk 35 \
  --tau_low 0.05 \
  --tau_high 0.20 \
...
Notes:
- This is not generation; Qwen3-Reranker provides a relevance score P(yes).
- Your existing enriched HPO definitions are used as candidate documents (hpo_def).
"""
...
class Qwen3Reranker:
    ...
    We use it as a CE reranker for MoRE refine:
      Query  = mention + context + task string
      Doc    = candidate HPO definition text (candidate["hpo_def"])
      score_i = P(yes)
      pred = argmax_i score_i
    """
...
def run_rerank_refine_for_dataset(
    ...
    """
    Margin-gated pipeline using local reranker:
    ...
    For each sample (gold ∈ topK):
      ...
      If margin <= tau_low:
          call reranker, trust reranker top1

      If tau_low < margin < tau_high:
          call reranker:
            compute rerank_margin = score_top1 - score_top2 over P(yes)
            if rerank_margin >= rerank_tau: accept reranker
            else: fallback to dual_best_id

    We track:
      - pipeline_top1_hits on n_samples
      - refine_calls
      - rerank_top1_hits on called samples (conditional)
    """
    ...
    for sample in tqdm(samples_for_refine, desc=f"[RERANK] {dataset_name} - gated refine", leave=False):
        ...
        # (helps decide acceptance in medium case)
        ...
        if margin <= tau_low:
            # Hard case: trust reranker
            pred_final = rerank_hid
        else:
            # Medium case: accept reranker only if confident
...
def plot_comparisons(
    ...
    # 2) Recall@K vs Rerank conditional
    plt.figure(figsize=(fig_w, 4.5))
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="DualLoRAEnc eval + LOCAL Qwen3-Reranker refine for HPO-ID (topK candidates) with margin-based gating (global HPO table)."
    )
    ap.add_argument(
        "--eval_roots",
        type=str,
...
def main():
    ...
    for root in eval_roots:
        ...
        # Pipeline top-1 over ALL spans (hits only counted within refine_samples, but denom is total_spans)
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            ...
            "Qwen3-Reranker (cross-encoder) candidate selector, using the Dual similarity margin "
            "to decide when to trust the encoder vs when to rely on the reranker.\n"
        )
